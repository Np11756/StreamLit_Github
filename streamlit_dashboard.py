...
st.set_page_config(page_title="Car Sales Forecasting Dashboard", layout="wide", initial_sidebar_state="expanded")
...
with tab1:
    ...
    for prefix, value in {"Dealer_Region": region, "Body Style": body_style, "Transmission": transmission}.items():
    ...
    with col1:
        st.metric("HistGradientBoosting Model", f"${hgb_pred:,.2f}")
    with col2:
        st.metric("Linear Regression Baseline", f"${lr_pred:,.2f}")
...
with tab2:
    ...
    filtered_df = df[df["Dealer_Region"] == selected_region[0]["x"]] if selected_region else df
    fig_map = px.scatter_mapbox(filtered_df, lat="Latitude", lon="Longitude", color="Dealer_Region",
